<?php
	 ...
	 * listing a collection of Confirmation objects.  It includes
	 ...
	 *
	 */
	class ConfirmationDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Confirmation datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Confirmation object, or it can be any QQNode extending from QQN::Confirmation().
		 *
		 ...
		 * sorting using two fields from the Confirmation object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Confirmation
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdConfirmation=<?=urlencode($_ITEM->IdConfirmation)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdConfirmation)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdConfirmation, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Confirmation::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Confirmation, given the clauses above
			$this->DataSource = Confirmation::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Confirmation-based QQNode.
		 * It will also verify that it is a proper Confirmation-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'confirmation') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "confirmation".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdConfirmation': return QQN::Confirmation()->IdConfirmation;
				case 'CreateDateTime': return QQN::Confirmation()->CreateDateTime;
				case 'FromSms': return QQN::Confirmation()->FromSms;
				case 'BodySms': return QQN::Confirmation()->BodySms;
				case 'CodeSms': return QQN::Confirmation()->CodeSms;
				case 'PhoneSms': return QQN::Confirmation()->PhoneSms;
				case 'EmailCreated': return QQN::Confirmation()->EmailCreated;
				case 'PasswordCreated': return QQN::Confirmation()->PasswordCreated;
				case 'IpCreated': return QQN::Confirmation()->IpCreated;
				case 'Verified': return QQN::Confirmation()->Verified;
				case 'DomainCreated': return QQN::Confirmation()->DomainCreated;
				default: throw new QCallerException('Simple Property not found in ConfirmationDataGrid content: ' . $mixContent);
