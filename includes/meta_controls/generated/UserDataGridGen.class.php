<?php
	...
	class UserDataGridGen extends QDataGrid {
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdUser=<?=urlencode($_ITEM->IdUser)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdUser)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdUser, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = User::QueryCount($objConditions);
			}
			...
			$this->DataSource = User::QueryArray($objConditions, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'IdUser': return QQN::User()->IdUser;
				case 'Email': return QQN::User()->Email;
				case 'Password': return QQN::User()->Password;
				case 'FirstName': return QQN::User()->FirstName;
				case 'MiddleName': return QQN::User()->MiddleName;
				case 'LastName': return QQN::User()->LastName;
				case 'Country': return QQN::User()->Country;
				case 'City': return QQN::User()->City;
				case 'Phone': return QQN::User()->Phone;
				case 'Birthday': return QQN::User()->Birthday;
				case 'YearGraduation': return QQN::User()->YearGraduation;
				case 'Cohort': return QQN::User()->Cohort;
				case 'ImageDriver': return QQN::User()->ImageDriver;
				case 'ImagePhoto': return QQN::User()->ImagePhoto;
				case 'MiningOption': return QQN::User()->MiningOption;
				case 'StatusUser': return QQN::User()->StatusUser;
				case 'Mac': return QQN::User()->Mac;
				case 'TokenMac': return QQN::User()->TokenMac;
				case 'StatusTokenMac': return QQN::User()->StatusTokenMac;
				case 'WalletAddress': return QQN::User()->WalletAddress;
				case 'NumberMasterNode': return QQN::User()->NumberMasterNode;
